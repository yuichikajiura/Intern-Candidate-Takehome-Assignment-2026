...
def build_output_path(
    ...
    run_ids_part = "-".join(str(rid) for rid in sorted(run_ids))
...
def plot_comparison(
    ...
    cells_sorted = sorted(exp_by_cell.keys())
...
def main() -> None:
    ...
    with connect_db(args.db_path) as conn:
        ...
        out_path = build_output_path(
            ...
            run_ids=[m.simulation_run_id for m in runs],
            cell_codes=list(exp_by_cell.keys()),
        ...
        print(
            ...
            f"(cells={sorted(exp_by_cell.keys())}, runs={[m.simulation_run_id for m in runs]})"
        )
